...
for(let i = 0; i < coreAddresses.length; i++) {
    ...
    core.on("ObjectiveTransfer", async (_from, _to, _value, _sig, _nonce) => {
        ...
        for(let j = 0; j < contracts.length; j++) {
            try {
                await contracts[j].objectiveTransfer(_from, _to, _value, _sig, _nonce);
            } catch (error) {
    ...
    core.on("ChallengedTransfer", async (_transfers) => {
        console.log("▩▩▩▩ CRITICAL ACTION REQUIRED ▩▩▩▩");
        console.log("> ✣ A transfer has been challenged!");
        console.log("> ❉ Relaying problematic transfers...");
        // The outer loop iterates over transfers not contracts to reduce stress
        // on a particular network/RPC. (If the outer loop iterated
        // over contracts, then there would be _transfers.length
        // requests to the same contract in a row.)
        for(let j = 0; j < _transfers.length; j++) {
            for(let k = 0; k < contracts.length; k++) {
                const challengeSuccess = contracts[k].tryChallenge(_transfers[j]);
                if(challengeSuccess) {
